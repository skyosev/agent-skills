...
- **Scope: package boundaries only.** Encapsulation, coupling, dependency direction, API surface. Do not flag type
  safety (→ invariant-hunter), type design (→ type-hunter), structural complexity (→ simplicity-hunter),
  interface design (→ solid-hunter), missing documentation (→ doc-hunter), security (→ security-hunter),
