...
- **Scope: invariant enforcement only.** Do not flag type design/architecture (→ type-hunter), package boundary issues
  (→ boundary-hunter), structural complexity (→ simplicity-hunter), interface design (→ solid-hunter), missing
  documentation (→ doc-hunter), security (→ security-hunter), test quality (→ test-hunter), or cosmetic style
  (→ slop-hunter). If a finding doesn't answer "is this invariant enforced?", it doesn't belong here.
