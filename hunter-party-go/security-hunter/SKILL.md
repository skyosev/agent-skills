...
- **Scope: security vulnerabilities only.** Do not flag type safety (→ invariant-hunter), type design (→ type-hunter),
  structural complexity (→ simplicity-hunter), package boundary issues (→ boundary-hunter), interface design
  (→ solid-hunter), missing documentation (→ doc-hunter), test quality (→ test-hunter), or cosmetic style
  (→ slop-hunter). If a finding doesn't answer "could this be exploited?", it doesn't belong here.
