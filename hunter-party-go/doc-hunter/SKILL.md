...
- **Scope: missing "why" documentation only.** Do not flag redundant or verbose comments (→ slop-hunter), structural
  complexity (→ simplicity-hunter), type safety (→ invariant-hunter), type design (→ type-hunter), package boundary
  issues (→ boundary-hunter), interface design (→ solid-hunter), security (→ security-hunter), or test quality
