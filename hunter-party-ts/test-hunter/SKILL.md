...
- **Scope: test quality and coverage only.** Do not flag production code issues — type invariants (→ invariant-hunter),
  type design (→ type-hunter), structural complexity (→ simplicity-hunter), module boundary issues (→ boundary-hunter),
  class/interface design (→ solid-hunter), missing documentation (→ doc-hunter), security (→ security-hunter), or
