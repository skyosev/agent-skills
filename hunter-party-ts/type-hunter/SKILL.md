...
- **Scope: type design and architecture only.** Do not flag type enforcement issues like loose optionality, unnecessary
  casts, or discriminated union enforcement (→ invariant-hunter), module boundary issues (→ boundary-hunter),
  class/interface design (→ solid-hunter), structural complexity (→ simplicity-hunter), missing documentation
